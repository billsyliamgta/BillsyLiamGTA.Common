...
{
    public class Scaleform : IDisposable
    {
        #region Properties
        ...
        private static void PushArgsInternal(params object[] args)
        {
            foreach (var arg in args)
            {
                switch (arg)
                {
                    ...
                    case string s:
                        {
                            Function.Call(Hash.BEGIN_TEXT_COMMAND_SCALEFORM_STRING, "STIRNG");
        ...
        public void Load(int timeout = 2000)
        {
            ...
            while (!HasLoaded)
            {
                ...
                {
                    throw new TimeoutException($"Scaleform '{Name}' failed to load within {timeout}ms.");
                }
                Script.Wait(0);
            }
        }
        ...
        public void DrawFullscreenMasked(Scaleform scaleform) => Function.Call(Hash.DRAW_SCALEFORM_MOVIE_FULLSCREEN_MASKED, Handle, scaleform.Handle, 255, 255, 255, 255);
