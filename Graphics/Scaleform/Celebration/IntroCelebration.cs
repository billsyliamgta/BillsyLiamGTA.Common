...
namespace BillsyLiamGTA.Common.Graphics.Scaleform
{
    public class IntroCelebration
    {
        ...
        public CelebrationTextureIds Texture { get; set; } = CelebrationTextureIds.BLANK;

        #endregion

        #region Constructor

        public IntroCelebration(string mode, string job, string challenge, CelebrationTextureIds texture = CelebrationTextureIds.BLANK)
        {
            Mode = mode;
            Job = job;
            Challenge = challenge;
        ...
        public void Show()
        {
            ...
            CallFunctionAll("ADD_BACKGROUND_TO_WALL", wallId, bgAlpha, (int)Texture);
