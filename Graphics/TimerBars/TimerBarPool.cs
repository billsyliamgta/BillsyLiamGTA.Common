...
namespace BillsyLiamGTA.Common.Graphics.TimerBars
{
    public class TimerBarPool : Script
    {
        ...
        {
            if (_bars?.Count > 0 && !Function.Call<bool>(Hash.IS_HUD_HIDDEN) && !Function.Call<bool>(Hash.IS_RADAR_HIDDEN) && !Function.Call<bool>(Hash.IS_CUTSCENE_PLAYING) && Game.Player.Character.IsAlive)
            {
